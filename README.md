...
- **regfile**: The MC3999's collection of registers. There are three flavors of registers contained in this module:
 ...
 - XBus commands: Chips are unable to do commands that use two XBus registers (e.g. mov x0 x1). The way our XBus logic is built, if such a command was run, it would be possible for the chip writing to x0 to get a read acknowledgement and continue before the chip reading from x1 to reach its read instruction. This is accidental, and was discovered after we concluded developement work. It's at the top of our list of recommended next steps.
 - Labels: Lines with only labels become a noop in machine code. This is a quirk of our compiler that could be removed, but it works in that labels in-game take up a line of memory, so having them take up a line of program memory is accurate in some ways, despite its behavior on execution being less accurate.
 - Limited command set: We deliberately chose not to implement a few commands (e.g. slx and tcp) for the sake of time and to simplify a couple aspects of our implementation. In the case of slx and tcp, this was because we decided that the functionality of these two commands already existed in other commands (slx is used in-game as a power use optimization, which we don't track, and tcp is very rarely used and is very nearly exactly the same as tgt). The commands dst and dgt, however, we chose not to implement both because they would be very difficult/awkward to define their behavior in hardware, as they interact directly with the digits of the base-10 value of numbers, and because dgt would require the use of three registers/immediates at once, and we felt it unnecessary to warp the design of the entire processor around one rarely used command.
